...
\noindent \textbf{\textit{API} (Application Programming Interface):} Conjunto de definiciones, protocolos y funciones que permite la comunicación e intercambio de datos entre diferentes componentes o servicios de software.  
\vspace{0.3cm}
...
\noindent \textbf{JWT:} Sigla de \textit{JSON Web Token}. Mecanismo de autenticación basado en tokens firmados digitalmente que permite transmitir información segura entre cliente y servidor de forma compacta y autónoma.
...
\noindent \textbf{RUP:} Sigla de \textit{Rational Unified Process}. Metodología de desarrollo de software iterativa e incremental que estructura el ciclo de vida del sistema en fases y disciplinas bien definidas.
...
\noindent \textbf{UX/UI:} Siglas de \textit{User Experience} y \textit{User Interface}. Conceptos asociados al diseño de sistemas interactivos que buscan optimizar la experiencia del usuario y la usabilidad de la interfaz gráfica.
\vspace{0.3cm}
...
La prueba \textit{Timed Up and Go} (\textit{TUG}) es una prueba funcional ampliamente utilizada para evaluar la movilidad básica de una persona mediante el tiempo requerido para levantarse de una silla, caminar una distancia de tres metros, realizar un giro de 180°, regresar al punto inicial y sentarse nuevamente \cite{Convergent-Validity-wearable-sensors}. Aunque el tiempo total de ejecución es el indicador más empleado en la práctica clínica, el análisis segmentado por subfases permite obtener información adicional al identificar en qué componentes del movimiento se concentra el desempeño del paciente, como las transiciones posturales, la marcha o los giros \cite{Association-Between-Performance-on-TUG}.\\

La Figura~\ref{subEtapasTug} ilustra esquemáticamente las diferentes subfases que componen la prueba \textit{TUG}, así como las principales variables biomecánicas y temporales asociadas a cada una de ellas. En esta figura, $\omega$ representa la velocidad angular del tronco alrededor del eje medio--lateral, mientras que $a$ corresponde a la aceleración en el eje antero--posterior. Ambas señales son registradas mediante sensores inerciales durante la ejecución de la prueba y permiten identificar eventos característicos del movimiento.\\

El tiempo $t$ se expresa en milisegundos (ms) y corresponde al eje temporal sobre el cual se analizan las señales. Los instantes marcados como $01$ y $02$ indican el punto de inicio de la prueba determinado por el observador 1 y el observador 2, respectivamente, utilizados para el análisis de concordancia interobservador.\\

En cuanto a las variables temporales por subfase, $t_{stb}$ denota el tiempo de transición de sedente a bípedo (levantarse de la silla), $t_{bts}$ corresponde al tiempo de transición de bípedo a sedente (sentarse), $t_{marcha}$ representa el tiempo total del periodo de marcha, y $t_{giro}$ es el tiempo empleado durante el giro de 180°. Finalmente, $t_{TUG}$ o $t_{tug}$ corresponde al tiempo total de ejecución de la prueba \textit{Timed Up and Go}.\\

La instrumentación del \textit{TUG} mediante sensores inerciales permite estimar de manera automática tanto el tiempo total de la prueba como los tiempos asociados a cada subfase, así como variables relacionadas con la aceleración y la rotación del tronco. Esta instrumentación incrementa la riqueza del análisis clínico y de investigación, al proporcionar información objetiva y reproducible sobre el desempeño funcional del paciente \cite{iTUG}.
...
\]
donde $M_{app,i}$ corresponde al valor medido por la aplicación desarrollada y $M_{ref,i}$ representa el valor medido por el sistema de referencia para la observación $i$.

El \textbf{sesgo} entre los métodos se define como el promedio de las diferencias:
\[
...
A partir de estos valores, los \textbf{límites de concordancia} al 95\% se definen como:
\[
LoA_{low} = \overline{d} - 1.96 \cdot SD_d, \qquad
LoA_{high} = \overline{d} + 1.96 \cdot SD_d,
\]
los cuales permiten evaluar la intercambiabilidad entre los métodos de medición en función de si el rango de variación observado es clínicamente aceptable \cite{BlandAltman1986}.



\subsection{Métricas de error (MAE, RMSE, MAPE)}

Para complementar el análisis de concordancia de Bland--Altman, se emplean métricas de error que cuantifican la magnitud de las diferencias entre los métodos. Sea $d_i$ la diferencia definida previamente y $n$ el número total de observaciones.

El \textbf{error absoluto medio} (MAE) se define como:
\[
MAE = \frac{1}{n}\sum_{i=1}^n |d_i|,
\]
y representa el error promedio en unidades de la variable medida.

El \textbf{error cuadrático medio} (RMSE) se calcula como:
\[
RMSE = \sqrt{\frac{1}{n}\sum_{i=1}^n d_i^2},
\]
penalizando en mayor medida los errores de mayor magnitud.

Por su parte, el \textbf{error porcentual absoluto medio} (MAPE) se define como:
\[
MAPE = \frac{100}{n}\sum_{i=1}^n \left|\frac{d_i}{M_{ref,i}}\right|,
\]
donde $M_{ref,i}$ corresponde al valor de referencia de la observación $i$. Esta métrica expresa el error en términos porcentuales y debe interpretarse con precaución en subfases de corta duración, dado que valores pequeños del denominador pueden amplificar el porcentaje de error.


\subsection{Planos Anatómicos}

Los planos anatómicos son divisiones imaginarias que se utilizan para describir la ubicación y el movimiento de las estructuras del cuerpo humano. Los tres planos principales son el plano sagital, el plano coronal (o frontal) y el plano transversal (o axial), como se observa en la Figura \ref{fig:anatomical-planes}. Estos planos permiten una mejor comprensión de la anatomía y la función del cuerpo, facilitando la comunicación entre profesionales de la salud.\\
...
El \textbf{error estándar de medición} (\textit{SEM}) cuantifica la imprecisión típica asociada a una medición y se define como:
\[
SEM = SD \cdot \sqrt{1 - \textit{ICC}},
\]
donde $SD$ corresponde a la desviación estándar de las mediciones y $\textit{ICC}$ es el coeficiente de correlación intraclase obtenido para la variable analizada.

A partir del \textit{SEM} se define el \textbf{cambio mínimo detectable} (MDC), el cual representa el umbral mínimo de cambio necesario para afirmar que una variación observada no se debe al error de medición. Para un nivel de confianza del 95\%, el MDC se calcula como:
\[
MDC_{95} = 1.96 \cdot \sqrt{2} \cdot SEM.
\]

Estas métricas son ampliamente utilizadas en estudios de medición clínica y rehabilitación para evaluar la confiabilidad y sensibilidad al cambio de los instrumentos de medición \cite{Weir2005}.
...
\begin{enumerate}
    ...
    \item \textbf{Pruebas de factibilidad en condiciones relevantes:} el sistema debe ser probado en un entorno de laboratorio con voluntarios, siguiendo un protocolo de uso reproducible y comparando resultados frente a un sensor de referencia (BTS GSensor) disponible en el laboratorio del Servicio de Rehabilitación Humana, en adelante, SERH.
...
Para contextualizar el diseño del sistema, se realizó una entrevista al personal de fisioterapia del laboratorio SERH de la Universidad del Valle. Las principales conclusiones para el diseño del sistema fueron:

\begin{itemize}
    \item La prueba \textit{TUG} se aplica de forma frecuente en población mayor, en particular a partir de los 55 años, con predominio de pacientes mujeres, coherente con el contexto clínico de evaluación de movilidad.
...
\begin{itemize}
    ...
    \item \textbf{Ejecución guiada del \textit{TUG}:} ofrecer un flujo que incluya verificación de sensores, calibración, indicación de inicio y finalización automática de la prueba.
    ...
    \item \textbf{Modo online y autenticación:} permitir inicio de sesión con credenciales, obtención de \textit{token} y envío autenticado de pruebas al servidor.
...
\begin{itemize}
    \item \textbf{Usabilidad:} interfaz amigable e intuitiva, siguiendo buenas prácticas UX/UI (User Experience / User Interface) para minimizar errores operativos.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=0.5\linewidth]{Images/SplashAutoTug.png}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=0.5\linewidth]{Images/HomeViewAutoTug.png}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=0.5\linewidth]{Images/HistorialPruebasAutoTug.png}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=0.5\linewidth]{Images/SettingsAutoTug.png}
...
La detección automática de las subfases de la prueba \textit{TUG} se realiza a partir del análisis conjunto de las señales $g_x(t)$ y $g_y(t)$ y de sus desplazamientos angulares integrados $\theta_x(t)$ y $\theta_y(t)$. El algoritmo se basa en la identificación de eventos característicos del movimiento humano, tales como cruces por umbral, picos de velocidad angular, cambios de signo y periodos de estabilización.

Las Figuras~\ref{fig:TestConCono} y~\ref{fig:TestSinCono} muestran dos ejemplos representativos de señales utilizadas para evaluar el comportamiento del algoritmo. En la Figura~\ref{fig:TestConCono}, el sujeto realiza un giro marcado alrededor del cono: durante la marcha se observa un pequeño giro inicial hacia un lado y posteriormente un giro más pronunciado en sentido contrario para bordear el cono. En contraste, en la Figura~\ref{fig:TestSinCono}, el sujeto camina directamente hacia un lado del cono, realizando únicamente un giro principal para rodearlo.

Esta diferencia en la estrategia de ejecución resulta crítica, ya que algoritmos previos dependían de la presencia de un giro inicial pequeño para detectar el inicio del primer giro. La lógica implementada en este trabajo elimina dicha dependencia, permitiendo una detección más robusta frente a variabilidad en el patrón de marcha.


\subsection{Sentado--parado}

La transición desde la posición sentada hasta la bipedestación se detecta a partir de la señal de velocidad angular $g_x(t)$, asociada principalmente a la flexión anterior del tronco durante el levantamiento. El inicio de la fase sentado--parado se define en el instante $t_{stb}^{ini}$ cuando $g_x(t)$ supera un umbral fijo positivo $\gamma_{stb} = 0.5~\text{rad/s}$ de manera sostenida, y simultáneamente el desplazamiento angular integrado $\theta_x(t)$ supera un valor mínimo $\Theta_{stb} = 0.1~\text{rad}$.

Formalmente, el evento de inicio se detecta cuando se cumplen las siguientes condiciones:
\[
g_x(t) > 0.5~\text{rad/s}, \qquad \theta_x(t) - \theta_x(t_0) > 0.1~\text{rad},
\]
donde $t_0$ corresponde a un instante previo al inicio del movimiento.

La combinación de ambos criterios permite descartar falsas detecciones debidas a pequeñas oscilaciones del tronco o ruido de alta frecuencia, ya que exige no solo un pico instantáneo de velocidad angular, sino un desplazamiento angular acumulado coherente con un movimiento real de levantamiento.

El final de la fase sentado--parado se identifica cuando $g_x(t)$ retorna a valores cercanos a cero y permanece dentro del intervalo $|g_x(t)| < 0.1~\text{rad/s}$ durante un tiempo mínimo de estabilización $\Delta t_{est} = 0.5~\text{s}$, indicando que el tronco ha alcanzado una postura erguida y estable.


\subsection{Marcha inicial}

La fase de marcha inicial comienza inmediatamente después de completarse la transición sentado--parado. En esta implementación, la detección de la marcha no depende de la presencia de un pico pronunciado en aceleración vertical, sino de la finalización estable de la fase anterior.

Se considera que la marcha inicia en el instante $t_{m1}^{ini}$ correspondiente al final del periodo de estabilización de $g_x(t)$ descrito previamente. Durante esta fase, las señales $g_x(t)$ y $g_y(t)$ permanecen cercanas a cero en promedio, reflejando un desplazamiento predominantemente traslacional sin rotaciones significativas del tronco.

El final de la marcha inicial se detecta cuando la magnitud de la velocidad angular $|g_y(t)|$ supera un umbral fijo $\gamma_{giro} = 0.4~\text{rad/s}$ de forma sostenida, indicando el inicio de una rotación axial del tronco asociada al primer giro.



\subsection{Primer giro}

El primer giro se detecta a partir de la señal $g_y(t)$, asociada a la rotación del tronco alrededor del eje vertical. El inicio del giro ocurre cuando $|g_y(t)|$ supera el umbral fijo $\gamma_{giro} = 0.4~\text{rad/s}$ y el desplazamiento angular integrado $\theta_y(t)$ comienza a aumentar o disminuir de forma monótona.

El sentido del giro se determina directamente a partir del signo del desplazamiento angular acumulado:
\begin{itemize}
    \item $\theta_y(t)$ creciente: giro en sentido antihorario.
    \item $\theta_y(t)$ decreciente: giro en sentido horario.
\end{itemize}

Adicionalmente, se exige que el valor absoluto del desplazamiento angular alcance un umbral mínimo $\Theta_{giro} = 0.5~\text{rad}$ para confirmar que la rotación corresponde a un giro real y no a una desviación leve durante la marcha.

El final del primer giro se define cuando $|g_y(t)|$ retorna al intervalo $|g_y(t)| < 0.1~\text{rad/s}$ y se mantiene estable durante al menos $\Delta t_{est} = 0.5~\text{s}$.



\subsection{Marcha de retorno}

La marcha de retorno se inicia inmediatamente después del final del primer giro. Durante esta fase, la señal $g_y(t)$ permanece cercana a cero en promedio, lo que indica ausencia de rotación significativa del tronco mientras el sujeto se desplaza nuevamente hacia la silla.

Formalmente, esta fase se caracteriza por el cumplimiento de:
\[
|g_y(t)| < 0.1~\text{rad/s},
\]
durante un intervalo continuo, lo cual permite distinguir la marcha de retorno de eventos de giro o ajustes posturales transitorios.

El final de la marcha de retorno se detecta cuando se observa un nuevo incremento sostenido en $|g_y(t)|$ que supera el umbral $\gamma_{giro}$, indicando el inicio del segundo giro.



\subsection{Segundo giro}

La detección del segundo giro incorpora un criterio adicional respecto al primer giro, basado en la coherencia del sentido de rotación. En primer lugar, se identifica el sentido del primer giro a partir del signo del desplazamiento angular $\theta_y(t)$. A partir de esta información, se espera que el segundo giro ocurra en el sentido opuesto, de acuerdo con la cinemática natural de la prueba \textit{TUG}.

El inicio del segundo giro se detecta cuando la derivada temporal del desplazamiento angular,
\[
\frac{d\theta_y(t)}{dt},
\]
supera un umbral fijo $\Gamma_{giro} = 0.3~\text{rad/s}$ con signo opuesto al observado en el primer giro. Este criterio permite identificar el giro de retorno de manera temprana y evita falsas detecciones asociadas a oscilaciones residuales tras la marcha.

Adicionalmente, se exige que el desplazamiento angular acumulado alcance nuevamente el umbral $|\Theta_{giro}| = 0.5~\text{rad}$ para confirmar que el evento corresponde a un giro efectivo y no a un ajuste postural menor.

El final del segundo giro se define cuando $|g_y(t)|$ retorna a valores inferiores a $0.1~\text{rad/s}$ y se mantiene estable durante un intervalo mínimo de $\Delta t_{est}$.



\subsection{Parado--sentado y finalización}

La transición final desde la posición de pie hasta sentado se detecta nuevamente a partir de la señal $g_x(t)$, asociada a la flexión anterior del tronco. El inicio de esta fase se define cuando $g_x(t)$ supera un umbral positivo moderado $\gamma_{bts} = 0.4~\text{rad/s}$ y el desplazamiento angular integrado $\theta_x(t)$ presenta un incremento coherente con el movimiento de descenso.

El final de la prueba se determina cuando todas las señales del giroscopio cumplen:
\[
|g_x(t)| < 0.1~\text{rad/s}, \qquad |g_y(t)| < 0.1~\text{rad/s},
\]
durante un intervalo continuo de al menos $5~\text{s}$. Este criterio garantiza que el paciente se encuentra completamente inmóvil y evita finalizar la prueba debido a pausas breves o movimientos residuales.



\section{Comparación con el algoritmo previo}

La lógica de segmentación implementada en este trabajo introduce cambios sustanciales respecto al algoritmo desarrollado en el trabajo previo. En primer lugar, se elimina la dependencia de giros iniciales pequeños para detectar el inicio del primer giro, lo cual generaba errores cuando el sujeto realizaba trayectorias asimétricas alrededor del cono.

Adicionalmente, se reduce el uso de múltiples banderas lógicas intermedias, reemplazándolas por criterios directos basados en umbrales, estabilidad temporal y análisis del desplazamiento angular y su derivada. Esto simplifica la estructura del algoritmo y mejora su interpretabilidad.

Finalmente, la separación explícita entre giros y fases de marcha, así como la redefinición del inicio y fin de la transición parado--sentado, permiten una segmentación temporal más precisa y consistente en condiciones reales de uso, incrementando la robustez del sistema frente a variabilidad intersujeto y diferencias en la ejecución motora.
...
El trabajo previo desarrollado por Pérez Kuleshova \cite{Sistema-portable-TUG-AKuleshov} estableció una base funcional para la detección de las subfases del \textit{TUG}. No obstante, durante las pruebas preliminares realizadas en este Trabajo de Grado se identificaron limitaciones asociadas a la sensibilidad a la orientación del dispositivo, dependencia excesiva de umbrales fijos y uso intensivo de banderas lógicas intermedias.
...
\begin{figure}[h!]
    \centering
    \includesvg[width=1\textwidth]{Images/Velocity_Angular_Position_Signals_1.svg}
...
\begin{figure}[h!]
    \centering
    \includesvg[width=1\linewidth]{Images/Velocity_Angular_Position_Signals_2.svg}
...
En la versión original, el inicio de la fase sentado--parado se detectaba mediante un umbral fijo aplicado sobre la señal de aceleración. En la versión actual, este criterio se refuerza exigiendo simultáneamente:
...
Este enfoque permite reducir la detección de falsos positivos asociados a oscilaciones de alta frecuencia y garantiza que el evento identificado corresponda a un movimiento real del cuerpo y no a ruido de la señal.

Asimismo, la detección de la transición hacia la marcha inicial se redefine empleando un patrón más simple basado en un cruce negativo de la aceleración antero--posterior, seguido de una fase de estabilización de dicha señal, eliminando la dependencia de picos positivos aislados que resultan poco robustos ante variabilidad intersujeto.
...
\begin{figure}[H]
    ...
    \caption{Reporte de análisis de los tiempos de las sub fases de la prueba \textit{Timed Up and Go.}}
...
Este capítulo presentó la arquitectura del servidor y la reformulación del procesamiento de datos del \textit{TUG} instrumentado. La adaptación de los algoritmos permitió mejorar la robustez de la segmentación temporal y habilitó el cálculo de variables biomecánicas adicionales, consolidando el sistema como una herramienta funcional y validable en condiciones reales.
...
En este capítulo se presentan las pruebas de validación realizadas para evaluar la validez por acuerdo y la confiabilidad inter-método de la aplicación móvil Android desarrollada para la prueba \textit{Timed Up and Go} (\textit{TUG}), utilizando como referencia un sensor inercial comercial BTS GSensor.\\
...
La confiabilidad inter-método se evaluó con el coeficiente de correlación intraclase bajo un modelo de dos vías con acuerdo absoluto \textit{ICC}(2,1). Se reportan dos análisis: (i) considerando cada ensayo como una unidad independiente (paciente--ensayo) y (ii) usando el promedio de ensayos por paciente, con el fin de observar la estabilidad de la medición al reducir variabilidad intra-sujeto.\\
...
Los resultados muestran \textbf{confiabilidad excelente} para el \textbf{tiempo total} del TUG (\textit{ICC}(2,1)$\approx 0.99$ en ambos enfoques), lo cual indica que la aplicación reproduce de manera consistente la medición del desenlace global respecto al sensor de referencia.\\

Para las subfases, se observaron valores que varían entre pobres y buenos. Este comportamiento es coherente con la naturaleza del problema: las subfases dependen de la detección precisa de eventos temporales (inicio/fin de cada fase), de modo que pequeños desplazamientos en la segmentación producen variaciones relevantes en fases de corta duración. En muestras pequeñas, además, los intervalos de confianza del \textit{ICC} tienden a ser amplios, por lo que la interpretación debe realizarse con cautela y preferiblemente acompañada de análisis de acuerdo.\\
...
Tercero, el tamaño muestral es reducido y la muestra es relativamente homogénea (adultos sanos). Esto limita la estabilidad estadística del \textit{ICC} en subfases y sugiere que futuros estudios deben incorporar participantes con alteraciones reales de marcha y un mayor número de ensayos.\\
...
Este anexo presenta la arquitectura general del sistema desarrollado en el presente trabajo de grado, el cual integra una aplicación móvil Android, un backend servidor y un microservicio especializado para el procesamiento de la prueba Timed Up and Go (\textit{TUG}).

El sistema sigue una arquitectura cliente--servidor con enfoque modular, donde cada componente cumple una función específica y se comunica mediante interfaces bien definidas. La aplicación móvil actúa como cliente, encargándose de la adquisición de señales inerciales y del envío de datos al servidor. El backend gestiona la autenticación, el almacenamiento y la orquestación de los microservicios, mientras que el microservicio \textit{TUG} se encarga del procesamiento de señales crudas y del cálculo de variables clínicas.
...
El microservicio del subsistema Timed Up and Go (\textit{TUG}) fue diseñado como un servicio independiente encargado exclusivamente del procesamiento de datos inerciales y del cálculo de variables de interés clínico.

El flujo de funcionamiento del microservicio se puede resumir en las siguientes etapas:

\begin{itemize}
    \item Autenticación del usuario mediante \textit{token} JWT validado por el backend.
    ...
    \begin{itemize}
        ...
        \item Ventanas temporales correspondientes a las fases del \textit{TUG}.
    \end{itemize}
    \item Preprocesamiento de señales (rotaciones, compensación de gravedad y filtrado).
    \item Segmentación temporal automática de las fases del \textit{TUG}.
...
El microservicio \textit{TUG} calcula automáticamente un conjunto de variables clínicas y biomecánicas a partir de las señales inerciales y de la segmentación temporal de la prueba.
...
Todas las variables se calculan únicamente dentro de las ventanas temporales correspondientes a cada fase del \textit{TUG}, garantizando consistencia temporal y evitando contaminación entre eventos.
...
El código fuente correspondiente a la aplicación móvil Android, al backend servidor y al microservicio del subsistema Timed Up and Go (\textit{TUG}) desarrollado en este trabajo de grado no se incluye de forma íntegra en este documento debido a su extensión.
